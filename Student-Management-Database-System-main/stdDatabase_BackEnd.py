...
global conn, cursor
conn = mysql.connector.connect(host='localhost', database='crime', user='root', password='root')
cursor = conn.cursor()
...
def add_crime_type():
    ...
    sql = 'insert into crime_type(offence_name,ipc_section,comment) values("{}","{}","{}");'.format(offence_name,
                                                                                                    ipc_section,
                                                                                                    comment)
    cursor.execute(sql)
...
def add_record():
    ...
    sql = 'insert into crime_record(c_date,offence_type,complaint_by,address,phone_no,status,update_date) values \
            ("{}",{},"{}","{}","{}","{}","{}");'.format(crime_date, offence_id, complaint_by, address, phone, status,
                                                        update_date)
    cursor.execute(sql)
...
def report_menu():
    ...
    if records < 1:
        ...
        print('-' * 100)
        print(
            '{} {} {} {} {} {} {} {}'.format('id', 'Crime Date', 'Crime', 'Complaint By', 'Address', 'Phone', 'Status',
                                             'Update On'))
        print('-' * 100)
        line = 1
        for row in results:
